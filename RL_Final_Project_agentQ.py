...
def get_state_index(x, y, z):
    x_idx = x
    y_idx = 7 * y  # Adjust to match environment's calculation
    z_idx = 49 * z  # Adjust to match environment's calculation
    return x_idx + y_idx + z_idx  # Adjusted to reflect the actual range of states


class QAgent:
    """Off-policy Q-Learning RL agent, updated for the TunnelRunner environment."""
    ...
    def __init__(self):
        ...
        self.expl_z = 0  # Explorer's z position now ranges from 0 to 6
    ...
    # Function to get the maximum Q-value for the next state.
    def max_Q(self, s):
    ...
    def select_action(self, state):
        ...
        if rng.random() < self.epsilon:
            ...
            return rng.integers(0, self.num_actions)
        else:
            # With probability 1 - epsilon, select the action with the highest Q-value (exploit)
    ...
    # Function to perform the Q-learning update for a state-action pair.
    def update_q_value(self, s, a, r, s_prime):
        # Apply the Q-learning update rule
        self.q[s, a] = self.q[s, a] + self.alpha * (r + self.gamma * self.max_Q(s_prime) - self.q[s, a])
